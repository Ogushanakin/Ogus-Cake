...
struct NetworkService {
    ...
    func placeOrder(dishId: String, name: String, completion: @escaping(Result<Library, Error>) -> Void) {
    ...
    private func request<T: Decodable>(route: Route,
        ...
        URLSession.shared.dataTask(with: request) { data, response,
            ...
            if let data = data {
                result = .success(data)
                let responseString = String(data: data, encoding: .utf8) ?? "Could not stringify out data"
    ...
    private func handleResponse<T: Decodable>(result: Result<Data,
        Error>?,
                                              completion: (Result<T,
                                                           Error>) -> Void) {
